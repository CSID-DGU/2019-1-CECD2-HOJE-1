...
const {regRead, TessNreg, regReset} = require('./main/FrameTest/regTojson');
    ...
    ipcMain.on('START_SEARCH', async (event, result) => { //검색 시작
        ...
        ipcMain.on('PATH', async (event, result) => {
            for (const tmp of result) {
                let searchResult = await Exec(tmp, ['.jpg', '.png', '.tif']); //함수 실행
            }
        })
    });
    ...
    xhr.onreadystatechange = async function () {
        try {
            /*if (xhr.readyState === 4 && this.status === 200) {
                ...
                let tmp = await makeDictionary(data, name, ppath, result1); //검사 결과를 딕션너리 형태로
                win.window.webContents.send('RESULT_DICTIONARY', tmp);
            } else if (this.readyState === 2 && this.status === 0) {
                check = false;
            }*/
            if (this.readyState === XMLHttpRequest.DONE) {
                if (this.status === 200) { //성공적으로 서버와 통신
                    data = xhr.responseText;
                    let tmp = await makeDictionary(data, name, ppath, result1); //검사 결과를 딕션너리 형태로
                    win.window.webContents.send('RESULT_DICTIONARY', tmp);
                } else {
